...
import { escapeHtml } from '@/lib/tiwa/sanitize';
import { siteConfig } from '@/lib/site';
import { sendTiwaSms } from '@/lib/sms-tiwa/client';
import { tiwaSmsOrderPlaced, tiwaSmsTrackingUrl, tiwaSmsAdminNewOrder } from '@/lib/sms-tiwa/messages';
...
const EMAIL_FROM =
    ...
    `Palm Shades <${siteConfig.contact.email}>`;
const BRAND = {
    name: siteConfig.name,
    shortName: siteConfig.shortName,
    tagline: 'Salon · Beauty · Retail',
    color: brandAssets.colors.pink,
    colorLight: brandAssets.colors.pinkLight,
    colorDark: brandAssets.colors.pinkDark,
    url: (process.env.NEXT_PUBLIC_APP_URL || process.env.NEXT_PUBLIC_SITE_URL || siteConfig.url).replace(/\/+$/, ''),
    phone: process.env.NEXT_PUBLIC_CONTACT_PHONE || siteConfig.contact.phones[0]?.tel || '+233242149489',
};
function smsPrefix(message: string): string {
    return `${BRAND.shortName}: ${message}`;
}
...
export async function sendSMS({ to, message }: { to: string; message: string }) {
    const result = await sendTiwaSms({ to, message });
    if (!result.ok) {
...
/** Email + SMS when a customer places an order (before or during payment). */
export async function sendOrderPlacedNotification(order: any, paymentUrl?: string) {
    ...
    const trackingUrl = tiwaSmsTrackingUrl(order_number || id);
...
<div style="text-align:center;margin-bottom:24px;">
  ...
  <h2 style="margin:0 0 4px;color:#111827;font-size:24px;">Order Received!</h2>
  <p style="margin:0;color:#6b7280;font-size:15px;">Thank you, ${name}.</p>
</div>
...
<p style="color:#374151;font-size:14px;line-height:1.6;margin:16px 0;">${
        paymentUrl
            ? 'Your order is reserved. Complete Mobile Money payment using the button below.'
            : "We've received your order and will notify you as it's processed."
    }</p>

${paymentUrl ? emailButton('Complete Payment', paymentUrl, BRAND.colorDark) : emailButton('Track Your Order', trackingUrl)}

<p style="color:#9ca3af;font-size:12px;text-align:center;margin:0;">Or copy this link: <a href="${paymentUrl || trackingUrl}" style="color:${BRAND.colorDark};">${paymentUrl || trackingUrl}</a></p>
`, `Your order #${order_number || id} was received`);

    await sendEmail({
        to: email,
        subject: `${BRAND.shortName} — Order Received · #${order_number || id}`,
        html: customerEmailHtml,
    });

    await sendEmail({
        to: ADMIN_EMAIL,
        subject: `${BRAND.shortName} — New Order · #${order_number || id}`,
        html: emailLayout(`
<h2 style="margin:0 0 16px;color:#111827;font-size:20px;">New order placed</h2>
...
${emailButton('View in Admin', `${BRAND.url}/admin/orders/${id}`)}
...
`, `Your order #${order_number || id} is confirmed!`);
    ...
    await sendEmail({
        to: email,
        subject: `${BRAND.shortName} — Order Confirmed · #${order_number || id}`,
        html: customerEmailHtml
    });
...
export async function sendOrderStatusUpdate(order: any, newStatus: string) {
    ...
    const subject = `${BRAND.shortName} — Order Update · #${order_number || id}`;
...
<p style="color:#374151;font-size:14px;line-height:1.7;margin:16px 0;">Thank you for joining the ${BRAND.name} family. Salon services, beauty products, and expert care &mdash; all in one place.</p>
...
`, `Welcome to ${BRAND.name}, ${firstName}!`)
    ...
    if (phone) {
        await sendSMS({
            to: phone,
            message: smsPrefix(`Welcome ${firstName}! Thanks for joining ${BRAND.name}.`)
        });
    }
}
...
export async function sendPaymentLink(order: any) {
    ...
    await sendEmail({
        to: email,
        subject: `${BRAND.shortName} — Complete Your Order · #${order_number}`,
        html: emailLayout(`
<div style="text-align:center;margin-bottom:24px;">
...
export async function sendContactMessage(data: { name: string, email: string, subject: string, message: string }) {
    ...
    await sendEmail({
        to: email,
        subject: `${BRAND.shortName} — We received your message: ${subject}`,
        html: emailLayout(`
<div style="text-align:center;margin-bottom:24px;">
  <div style="width:64px;height:64px;background-color:${BRAND.colorLight};border-radius:50%;margin:0 auto 16px;line-height:64px;font-size:28px;">&#128172;</div>
...
`, `Thanks for contacting us, ${safeName}`)
    ...
    await sendEmail({
        to: ADMIN_EMAIL,
        subject: `${BRAND.shortName} — Contact: ${subject}`,
        html: emailLayout(`
