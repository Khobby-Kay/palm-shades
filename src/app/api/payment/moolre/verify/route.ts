...
import { sendOrderConfirmation } from '@/lib/tiwa/notifications';
...
export async function POST(req: Request) {
    try {
        ...
        // 8. Keep Prisma storefront order in sync (Palm Shades account orders)
        try {
            const prismaOrder = await prisma.order.findUnique({
                where: { orderNumber },
                select: { id: true, paymentStatus: true },
            });
            if (prismaOrder && prismaOrder.paymentStatus !== 'PAID') {
                await markOrderPaid(prismaOrder.id, 'moolre-api-verify');
            }
        } catch (prismaErr: any) {
            console.error('[Verify] Prisma sync failed:', prismaErr.message);
        }
