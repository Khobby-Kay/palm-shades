...
import { sendOrderConfirmation, sendOrderStatusUpdate, sendWelcomeMessage, sendContactMessage, sendPaymentLink, sendEmail, sendSMS, emailLayout } from '@/lib/tiwa/notifications';
...
function brandedSubject(subject: string): string {
    ...
    return subject.startsWith(siteConfig.shortName) ? subject : `${prefix}${subject}`;
}

function brandedSms(message: string): string {
    const prefix = `${siteConfig.shortName}: `;
    return message.startsWith(siteConfig.shortName) ? message : `${prefix}${message}`;
}
