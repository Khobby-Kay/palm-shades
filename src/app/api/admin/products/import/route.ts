...
import { extractFromZip } from '@/lib/tiwa/import/zip-extractor';
import { parseCSV, type ValidationWarning } from '@/lib/tiwa/import/csv-parser';
import { uploadProductImages, collectReferencedImageNames } from '@/lib/tiwa/import/image-processor';
import {
  buildCategoryLookup,
...
} from '@/lib/tiwa/import/product-creator';
...
export async function POST(request: Request) {
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      const catalogSync = await syncCatalogFromSupabase();
      try {
        revalidatePath('/');
        revalidatePath('/shop');
        revalidatePath('/shop/[slug]', 'page');
      } catch (_) {}
      send('catalog_sync', catalogSync);
