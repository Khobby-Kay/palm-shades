...
export default function InventoryManagementPage() {
  ...
  const toExportRows = (list: typeof products): InventoryExportRow[] =>
    list.map((p) => ({
      sku: p.sku,
      name: p.name,
      category: p.category,
  ...
  const handleExportCsv = () => {
    exportInventoryCsv(toExportRows(getExportList()));
  };
  ...
  const handleExportPdf = () => {
    ...
    const label =
      selectedProducts.length > 0
        ? `Selected Inventory (${list.length})`
        : stockFilter === 'all'
          ? 'Inventory Report'
          : `Inventory — ${stockFilter} stock`;
    exportInventoryPdf(toExportRows(list), label);
  };
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between space-y-4 md:space-y-0">
            ...
            <div className="flex items-center space-x-3">
              ...
              <ExportDropdown
                ...
                className="[&_button]:px-4 [&_button]:py-3"
              />
            </div>
          </div>
