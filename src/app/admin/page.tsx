        ...
        if (recentOrdersData) {
          const formattedRecent = recentOrdersData.map((o: any) => {
            ...
            const customerName = (addr.firstName && addr.lastName)
              ...
              : addr.full_name || addr.firstName || emailLocalPart;
            return {
              id: o.id,
                  ...
                  <tbody>
                    {recentOrders.map((order) => (
                      <tr key={order.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                        ...
                        <td className="py-4 px-4 font-semibold text-gray-900 whitespace-nowrap">GH₵ {(Number(order.total) || 0).toFixed(2)}</td>
                        <td className="py-4 px-4">
            ...
            {topProducts.map((product) => (
              <div key={product.id} className="border border-gray-200 rounded-xl p-4 hover:shadow-md transition-shadow">
                <div className="relative mb-3 aspect-square overflow-hidden rounded-lg bg-gray-100">
                  <CoverImage src={product.image} alt={product.name} sizes="(max-width: 768px) 50vw, 20vw" />
                </div>
