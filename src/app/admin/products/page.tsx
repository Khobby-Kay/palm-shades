...
/** Full catalog sync in background (after delete — do not block the UI). */
function syncStorefrontInBackground() {
  void fetch('/api/admin/sync/catalog', {
    method: 'POST',
    credentials: 'include',
  }).catch((err) => console.warn('[products] storefront sync:', err));
}
                ...
                {filteredProducts.map((product) => (
                  <tr key={product.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                    ...
                    <td className="py-4 px-4">
                      <div className="flex items-center space-x-3">
                        <div className="relative h-16 w-16 overflow-hidden rounded-lg border border-gray-200 bg-gray-100">
                          <CoverImage src={product.image} alt={product.name} sizes="64px" />
                        </div>
                        <div>
            ...
            {filteredProducts.map((product) => (
              <div key={product.id} className="border-2 border-gray-200 rounded-xl p-4 hover:shadow-lg transition-shadow">
                <div className="relative">
                  ...
                  />
                  <div className="relative mb-3 aspect-square overflow-hidden rounded-lg border border-gray-200 bg-gray-100">
                    <CoverImage src={product.image} alt={product.name} sizes="(max-width: 768px) 50vw, 25vw" />
                  </div>
                </div>
