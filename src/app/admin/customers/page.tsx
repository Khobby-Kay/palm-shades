...
export default function AdminCustomersPage() {
  ...
  const toExportRows = (list: typeof customers): CustomerExportRow[] =>
    list.map((c) => ({
      name: c.name,
      email: c.email,
      phone: c.phone,
  ...
  const handleExportAllCsv = () => {
    exportCustomersCsv(toExportRows(filteredCustomers));
  };

  const handleExportAllPdf = () => {
    exportCustomersPdf(toExportRows(filteredCustomers));
  };

  const handleExportSelectedCsv = () => {
    const selected = filteredCustomers.filter((c) => selectedCustomers.includes(c.id));
    exportCustomersCsv(toExportRows(selected), `motchis-customers-selected-${selected.length}.csv`);
  };

  const handleExportSelectedPdf = () => {
    const selected = filteredCustomers.filter((c) => selectedCustomers.includes(c.id));
    exportCustomersPdf(toExportRows(selected), `Selected Customers (${selected.length})`);
  };
