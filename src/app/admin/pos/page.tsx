        ...
        <div className="relative">
            {completedOrder && (
                <div className="print-section hidden print:block bg-white p-8">
                    <MotchisPrintChrome
                        documentTitle={completedOrder.paymentPending ? 'Payment Pending Receipt' : 'POS Receipt'}
                        reference={completedOrder.orderNumber}
                    >
                        <div className="mb-4">
                            <p><span className="font-semibold">Customer:</span> {completedOrder.customerName || 'Walk-in'}</p>
                            <p><span className="font-semibold">Payment:</span> {completedOrder.paymentMethod}{completedOrder.paymentPending ? ' (pending)' : ''}</p>
                            <p><span className="font-semibold">Fulfillment:</span> {completedOrder.deliveryMethod === 'doorstep' ? 'Delivery' : 'Pickup'}</p>
                        </div>
                            ...
                            <tbody>
                                {completedOrder.items?.map((item: CartItem) => (
                                    <tr key={item.id} className="border-b border-gray-200">
                                        ...
                                        <td className="py-2 px-2 text-center">{item.cartQuantity}</td>
                                        <td className="py-2 px-2 text-right">GH₵ {(item.price * item.cartQuantity).toFixed(2)}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
                        ...
                        <div className="text-right space-y-1 mb-4">
                            <p>Subtotal: GH₵ {completedOrder.subtotal?.toFixed(2)}</p>
                            <p>Tax: GH₵ {completedOrder.tax?.toFixed(2)}</p>
                            <p className="font-bold text-lg border-t border-gray-400 pt-1">Total: GH₵ {completedOrder.total?.toFixed(2)}</p>
                            {completedOrder.changeDue > 0 && (
                                <p>Change due: GH₵ {completedOrder.changeDue.toFixed(2)}</p>
                            )}
                        </div>

                        {completedOrder.paymentPending && completedOrder.paymentUrl && (
                            <p className="text-xs break-all text-gray-600">Pay link: {completedOrder.paymentUrl}</p>
                        )}
                    </MotchisPrintChrome>
                </div>
            )}

        <div className="no-print flex flex-col lg:flex-row h-[calc(100vh-90px)] -m-4 lg:-m-6 overflow-hidden bg-gray-100 relative">
                    ...
                    {loading ? (
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 pb-20 lg:pb-4">
                            {filteredProducts.map(product => (
                                ...
                                >
                                    <div className="relative aspect-square shrink-0 bg-gray-50">
                                        <CoverImage
                                            src={product.image}
                                            alt={product.name}
                        ...
                        cart.map(item => (
                            <div key={item.id} className="flex gap-3 p-3 bg-gray-50 rounded-lg group hover:bg-gray-100 transition-colors">
                                <div className="relative h-16 w-16 flex-shrink-0 overflow-hidden rounded-md border border-gray-200 bg-white">
                                    <CoverImage src={item.image} alt={item.name} sizes="64px" />
                                </div>
