...
export default function ModulesPage() {
  ...
  const toggleModule = async (id: string, currentState: boolean) => {
    ...
    try {
      ...
      window.dispatchEvent(new CustomEvent(STORE_MODULES_CHANGED));
    } catch (err) {
      console.error('Error updating module:', err);
      setModules((prev) =>
        prev.map((m) => (m.id === id ? { ...m, enabled: currentState } : m))
      );
      alert('Failed to update settings. Check you are signed in as admin.');
    } finally {
    ...
    <div className="min-h-screen bg-gray-50 p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <p className="text-gray-600 mb-8">
          Enable or disable optional admin features. The sidebar updates instantly — no page reload.
        </p>
        <p className="text-sm text-gray-500 mb-8 -mt-6">
          Each toggle unlocks its admin page. Storefront pages are not modified by these toggles.
        </p>
                    ...
                    <div key={module.id} className={`bg-white rounded-xl border-2 p-6 transition-all ${module.enabled ? 'border-blue-500 shadow-md' : 'border-gray-200 opacity-75'}`}>
                      <div className="flex justify-between items-start mb-4">
                        ...
                        <button
                          ...
                          aria-busy={savingModuleId === module.id}
                          className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors disabled:opacity-60 ${module.enabled ? 'bg-blue-600' : 'bg-gray-300'}`}
                        >
