...
export default function AdminOrdersPage() {
  ...
  const handleBulkAction = async (action: string, newStatus?: string) => {
    if (newStatus) {
      ...
      exportOrdersCsv(toOrderExportRows(ordersToExport), 'motchis-orders-selected.csv');
    }
  };

  const toOrderExportRows = (list: Order[]): OrderExportRow[] =>
    list.map((o) => ({
      orderId: o.order_number || o.id,
      customer: getCustomerName(o),
      email: getCustomerEmail(o),
  ...
  const handleExportCsv = () => {
    ...
    exportOrdersCsv(toOrderExportRows(list));
  };
  ...
  const handleExportPdf = () => {
    ...
    const title =
      selectedOrders.length > 0
        ? `Selected Orders (${list.length})`
        : orderViewTab === 'confirmed'
          ? 'Confirmed Orders'
          : 'Abandoned Carts';
    exportOrdersPdf(toOrderExportRows(list), title);
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between space-y-4 md:space-y-0">
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center">
              <button
                ...
                className="min-h-[44px] w-full px-4 py-3 border-2 border-gray-300 text-gray-700 rounded-lg hover:border-gray-400 transition-colors font-medium whitespace-nowrap cursor-pointer sm:w-auto"
              >
              ...
              <select
                ...
                className="min-h-[44px] w-full px-4 py-3 pr-8 border-2 border-gray-300 rounded-lg text-base focus:ring-2 focus:ring-blue-500 focus:border-blue-500 font-medium cursor-pointer sm:w-auto sm:min-w-[10rem]"
              >
        ...
        {selectedOrders.length > 0 && (
          <div className="flex flex-col gap-3 border-b border-blue-200 bg-blue-50 p-4 sm:flex-row sm:items-center sm:justify-between">
